...
const ModalPage = (props) => {
  ...
  const addWord = () => {
    if (text.length == 0 || translate.length == 0) {
      return Alert.alert("Warn!", "Please Enter Write Word And Translate");
    }
    
    if (text.length > 30 || translate.length > 25){
      return Alert.alert("Warn!", "The Word Or Translate Is Very Long");
    }else {
      try {
        db.transaction((tx) => {
          tx.executeSql(
            ...
            (txObj, resultSet) => {
              if (resultSet.rows._array[0] === undefined) {
                tx.executeSql(
                  "INSERT INTO word (word , translate , category_id ) values (? ,? , ? )",
                  [text, translate, props.ctgId],
                  (txObj, resultSet) => {
                    setText("");
                    setTranslate("");
                    Alert.alert("Successfuly!", "The Word Added Successfuly");
                    setModalVisible(!modalVisible);
                  },
                  (txObj, error) => console.warn(error)
                );
              } else {
                Alert.alert("Warn!", "There Is Same Word");
                setText("");
                setTranslate("");
                setModalVisible(!modalVisible);
              }
